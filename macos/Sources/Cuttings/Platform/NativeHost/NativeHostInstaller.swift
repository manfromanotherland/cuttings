...
/// Installs the bundled `cuttings-native-host` binary's native-messaging manifest into
/// every browser found on the machine.
///
/// Called on every launch (from `boot`). The Rust `cuttings-native-host --install-manifest`
...
enum NativeHostInstaller {
    ...
    static func install() -> Bool {
        guard let hostURL = bundledHostURL() else {
            print("NativeHostInstaller: cuttings-native-host binary not found in bundle")
            return false
        }
    ...
    static func bundledHostURL() -> URL? {
        Bundle.main.bundleURL
            .appendingPathComponent("Contents/MacOS/cuttings-native-host")
...
enum NativeHostInstallerError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .nonZeroExit(code, output):
            "cuttings-native-host exited with code \(code): \(output)"
        }
    }
}
