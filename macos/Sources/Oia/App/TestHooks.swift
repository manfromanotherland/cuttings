...
enum TestHooks {
    ...
    static var libraryHydrationDelayNanoseconds: UInt64? {
        env("OIA_TEST_LIBRARY_HYDRATION_DELAY_MS")
            .flatMap(UInt64.init)
            .map { $0 * 1_000_000 }
    }
    ...
    static var libraryReconciliationDelayNanoseconds: UInt64? {
        env("OIA_TEST_LIBRARY_RECONCILIATION_DELAY_MS")
            .flatMap(UInt64.init)
            .map { $0 * 1_000_000 }
    }
    ...
    static var trustedCachedLibraryPath: String? {
        env("OIA_TEST_TRUSTED_CACHE_LIBRARY")
    }
    ...
    static func recordStartupEvent(_ event: String, details: String? = nil) {
        ...
        if let directory = env("OIA_TEST_STARTUP_EVENTS_DIR") {
    ...
    static func recordStartupSnapshot(_ event: String, rows: [ReadingRow]) {
        guard env("OIA_TEST_STARTUP_EVENTS_DIR") != nil else { return }
        let details = [
            String(rows.count),
            rows.first?.id ?? "",
