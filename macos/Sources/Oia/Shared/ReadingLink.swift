...
enum ReadingLink {
    /// UTM parameters identifying Óia as the referrer, so sites can see
    /// the traffic came from the app.
    private static let utmParameters = [
        URLQueryItem(name: "utm_source", value: "oia")
    ]

    /// Returns `url` with Óia' UTM reference appended. Existing query
