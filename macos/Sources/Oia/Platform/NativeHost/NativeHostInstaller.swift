...
/// Installs the bundled `oia-native-host` binary's native-messaging manifest into
/// every browser found on the machine.
///
/// Called on every launch (from `boot`). The Rust `oia-native-host --install-manifest`
...
enum NativeHostInstaller {
    ...
    static func install() -> Bool {
        guard let hostURL = bundledHostURL() else {
            print("NativeHostInstaller: oia-native-host binary not found in bundle")
            return false
        }
    ...
    static func bundledHostURL() -> URL? {
        Bundle.main.bundleURL
            .appendingPathComponent("Contents/MacOS/oia-native-host")
...
enum NativeHostInstallerError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .nonZeroExit(code, output):
            "oia-native-host exited with code \(code): \(output)"
        }
    }
}
