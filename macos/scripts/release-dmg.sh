#!/usr/bin/env bash
# Build Óia.app in Release, sign it with your Apple Developer ID, then
...
APP_NAME="Óia"
SCHEME="Oia"
PROJECT="Oia.xcodeproj"
BUILD_DIR="$ROOT/build"
STAGING="$ROOT/dist/dmg-staging"
DIST="$ROOT/dist"
DMG_PATH="$DIST/${APP_NAME}.dmg"
ENTITLEMENTS="$ROOT/Sources/Oia/App/Oia.entitlements"
...
# distribution. Guards: oia-native-host and a Frameworks dir may legitimately be
# absent, and an unguarded miss would abort under `set -e`.
if [ -f "$APP/Contents/MacOS/oia-native-host" ]; then
  sign "$APP/Contents/MacOS/oia-native-host"
fi
...
until hdiutil create -volname "$APP_NAME" -srcfolder "$STAGING" -ov -format UDZO "$DMG_PATH"; do
  if [ "$attempt" -ge 3 ]; then
    ...
    echo "       Close any Finder window on the Óia volume, then check:" >&2
    echo "         hdiutil info   # detach any stale Óia image" >&2
    exit 1
  fi
