#!/usr/bin/env bash
# Build Óia.app in Release and wrap it in a distributable .dmg.
#
...
#   xattr -dr com.apple.quarantine /Applications/Óia.app
#
...
# oia-native-host binary and a Frameworks dir may legitimately be absent (e.g. a
...
if [ -f "$APP/Contents/MacOS/oia-native-host" ]; then
  codesign --force --timestamp=none -s - "$APP/Contents/MacOS/oia-native-host"
fi
...
until hdiutil create -volname "$APP_NAME" -srcfolder "$STAGING" -ov -format UDZO "$DMG_PATH"; do
  if [ "$attempt" -ge 3 ]; then
    ...
    echo "       Close any Finder window on the Óia volume; see 'hdiutil info'." >&2
    exit 1
  fi
