...
struct LaunchOptions {
    /// Boots the app straight against this library (sets `OIA_TEST_LIBRARY`).
    var libraryPath: String?

    /// Redirects the index DB here (sets `OIA_TEST_DB`).
    var dbPath: String?

    /// Makes onboarding's "Choose Library…" pick this folder without an
    /// `NSOpenPanel` (sets `OIA_TEST_ONBOARDING_PICK`).
    var onboardingPickPath: String?

    /// Throwaway `UserDefaults` suite the app persists preferences to (sets
    /// `OIA_TEST_DEFAULTS`), so a test never touches the real defaults domain.
...
enum AppLauncher {
    ...
    static func launch(_ options: LaunchOptions = LaunchOptions()) -> XCUIApplication {
        ...
        if let libraryPath = options.libraryPath {
            environment["OIA_TEST_LIBRARY"] = libraryPath
        }
        if let dbPath = options.dbPath {
            environment["OIA_TEST_DB"] = dbPath
        }
        if let pick = options.onboardingPickPath {
            environment["OIA_TEST_ONBOARDING_PICK"] = pick
        }
        if let suite = options.defaultsSuite {
            environment["OIA_TEST_DEFAULTS"] = suite
        }
