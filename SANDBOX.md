...
A single reusable image that pre-installs the toolchains for the Óia
...
```bash
docker build -t oia/sandbox:1 .
docker image save oia/sandbox:1 -o rc-sandbox.tar
...
```bash
sbx run --template oia/sandbox:1 claude
```
...
- **Memory:** the SwiftSyntax compile is memory-hungry. The build serializes it
  ...
  `docker build --build-arg SWIFT_BUILD_JOBS=4 -t oia/sandbox:1 .`
